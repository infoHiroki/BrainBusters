...
開発時はタイトル画面に「デバッグ」ボタンが表示される（`__DEV__`フラグで制御）

### 機能
- **バトルテスト**: 階層・敵数・敵タイプ（通常/エリート/ボス）を指定してバトル開始
- **報酬画面テスト**: 通常報酬・ボス報酬をテスト
- **ゲーム設定確認**: GAME_CONFIGの値を確認
- **敵情報**: 各階層の敵種類数を確認

### 本番リリース
`App.tsx`の`DEV_MODE`を`false`にするか、プロダクションビルドで自動的に非表示
...
- タスクは`temp/CURRENT_TASKS.md`で管理
- **完了済みタスクは削除し、次にやることだけ書く**
- 引き継ぎ情報（プロジェクト概要・現在の状態・重要ファイル）は残す
