...
### Phase 1: エネルギー・カードバランス改善（優先度: 高）
**問題**: カードを全部使い切れる → 選択がない → 連打ゲームになっている

- [ ] エネルギーコストの幅を拡大（現状0-3 → 0-5or6程度）
- [ ] 最大エネルギーを増加（現状4 → 6-8程度）
- [ ] 0コストカードをさらに削減（希少に）
- [ ] 高コスト・高威力カードを追加
- [ ] 「どのカードを使うか」の取捨選択が発生するバランスに調整

**目標**: 手札6枚に対して3-4枚しか使えないバランス → 毎ターン選択が必要

### Phase 1.5: 難易度調整（優先度: 高）
- [ ] 敵のダメージをさらに調整
- [ ] 敵のHPをさらに調整
- [ ] 「防御しないと死ぬ」緊張感の確認
...
> - 手札を全部使える → 選択がない → 連打ゲーム
> - エネルギーコスト0-3の幅が狭すぎる
> - 考えずに出せる → 意思決定がない
> - 失敗しない → リスクがない

### 解決の方向性
1. **Phase 1** でエネルギー・コストバランスを調整し「選択」を生む
2. **Phase 1.5** で難易度を上げ「防御が必要」な状況を作る
3. **Phase 2** でタグシステムによりシナジーを追加
4. **Phase 3** でリスク/リターンの選択を追加

### バランス調整の指針
- **目標**: 手札6枚中、3-4枚しか使えない
- **最大エネルギー**: 6-8程度
- **コスト分布**: 0コスト（5%）、1-2（40%）、3-4（40%）、5-6（15%）
- **結果**: 「このターンは攻撃重視？防御重視？」の判断が必要に
