...
| 難易度 | 敵ダメージ2倍・敵HP1.5倍・プレイヤーHP100・調和強化 |
...
### 1. カードが選べないバグ（要観察）
- **症状**: 残エネルギーがあるのにカードが選べない時がある
- **可能性**:
  - 攻撃カード: 全敵HPが0以下でターゲットがいない（勝利待機中）
  - HPコストカード: プレイヤーHP不足で使用不可
  - `isProcessing`フラグ: タイミング問題でtrueのまま
  - 自動ターン終了: 800msの遅延中に状態が変わる
- **ステータス**: 再現条件の特定が必要

### 2. カードランダム性の問題（要観察）
- **症状**: 出てくるカードがランダムじゃない気がする
- **原因候補**:
  - `seed`がRunStateに保存されているが、実際の乱数生成に使用されていない
  - レアリティ別のカードプールの偏り（★1: 40%、★5: 2%）
  - カード効果の決定がID基準で決定論的（`getCardVariant(id) = id % 20`）
- **対応**: 疑似乱数シードの導入検討、またはプール分布の見直し

### 3. ダメージテキスト重複バグ ✅修正済み
- **症状**: 「45ダメージ、45ダメージ」のようにテキストが被って表示される
- **修正内容**: 敵の攻撃時のメッセージを1つに統合（攻撃力・防御・ダメージを1行で表示）
- **例**: 「敵名の攻撃! 🛡️5防御 → 💥15ダメージ」
...
- **原因**: BattleScreen.tsx内の`selectNextIntent`がハードコードされた値を使用していた
  - 敵のテンプレート（enemies.ts）を参照せず、固定のattack/defend/buff/debuffを返していた
- **修正内容**: enemies.tsの`selectNextIntent`をインポートして使用するように変更
  - 敵ごとの正しい行動パターンが使用されるようになった

### 6. 報酬画面の進行バグ（要ダブルチェック）
- **症状1**: 報酬画面から先に進めなくなる
...
- **修正内容**:
  - 敗北時に「💀 {敵名}の攻撃で倒れた...」というメッセージを表示
  - 画面シェイクを追加（ドラマチック演出）
  - 敗北確定から画面遷移まで1.5秒の遅延を追加（状況把握のため）
...
- [ ] カード一覧表示（未発見はシルエット）
- [ ] 発見率表示
- **※ 図鑑機能は他の機能（最終ボス報酬、カード管理）の前提となる**

### 最終ボス報酬の見直し
- **問題点**:
  - 50階ボス撃破後にカードを選べるのが意味不明
  - レリックをもらっても使う機会がない
  - 報酬がないと達成感が薄い
- **案**:
  - 案1: メダル/トロフィー獲得（図鑑に記録）
  - 案2: 実績解除＋エンディング演出
  - 案3: 次回ランへの引き継ぎ特典
  - 案4: 特別なカードスキン解放
- **依存**: 図鑑機能の後に検討
...
### カード管理・引き継ぎシステム
- [ ] **ストックカード選択スタート**
  - クリア済みカードから5枚選んで新ランを開始
  - 図鑑機能との連携が必要
  - カード管理画面の実装が必要
- [ ] **次回ラン引き継ぎ**
  - 最終ボスクリア後、ストックカードを次回初戦で使用可能
  - あまり意味がないかも？要検討
- [ ] **カードコレクション管理画面**
  - 所持カード一覧
  - お気に入り登録
